...
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var print = function (x) {    return x;};
   var $eval = function (x) {    return x;};
   var read = function (x) {    return x;};
   var rep = function (x) {    return print($eval(read(x)));};
   var body = A2($Console._op[">>="],
   A2($Console._op[">>>"],$Console.putStr("user> "),$Console.getLine),
   function (s) {
      return A2($Console._op[">>>"],$Console.putStrLn(rep(s)),body);
   });
   var loop = $Console.forever(body);
   var runner = Elm.Native.Task.make(_elm).performSignal("runner",$Console.run(loop));
   return _elm.Main.values = {_op: _op,read: read,$eval: $eval,print: print};
};
(function(){
